...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        return len(state[1]) == 4 #State all four corners in the pacman 
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if hitsWall != True:
                temp = list(state[1])
                sucpos = (nextx, nexty)
                cornerremain = self.corners
                if sucpos in cornerremain:
                    if sucpos not in state[1]:
                        newState = (sucpos, temp)
                        temp.append(sucpos)
                successors.append(((sucpos, temp), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for edge in corners:
        ...
        if edge not in state[1]:
            visits.add(edge)

    while visits:
        space = []
        for edge in visits:
        	m = util.manhattanDistance(unvisit, edge)
        	space.append((m, edge))
        dis, edge = min(space) #tupels arranged
...
def foodHeuristic(state, problem):
    ...
    for dot1 in food:
        for dot2 in food:
            maxcost = util.manhattanDistance(dot1, dot2) 
            if (maxcost > cost):
                cost = maxcost
                xcord = dot1
                ycord = dot2

    product = min(util.manhattanDistance(position, xcord), util.manhattanDistance(position, ycord)) + cost
    return product
