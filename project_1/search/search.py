    ...
    while frontier.isEmpty() != True:
        ...
        if cur[0] not in end:
            ...
            for successor in problem.getSuccessors(cur[0]):
            	state = successor[0]
            	move = [successor[1]]
            	frontier.push((state, cur[1] + move))
    return []
...
def uniformCostSearch(problem):
    ...
    while frontier is not True:
        node = frontier.pop()
        
        if node[0] not in visitedSet:
    ...
    while frontier is not True:
        ...
        if node[0] not in visitedSet:
            ...
            if problem.isGoalState(node[0]):
                ...
                return state
            visitedSet.append(node[0])
            
            for suc in problem.getSuccessors(node[0]):
            	h = heuristic(suc[0], problem)
            	frontier.push((suc[0], node[1] + [(suc[1])]), problem.getCostOfActions(node[1] + [(suc[1])]) + h)
